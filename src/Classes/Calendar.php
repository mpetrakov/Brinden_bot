...
class Calendar
{
    public function getCalendar(int $month, int $year): array
    {
        ...
        foreach (new DatePeriod($start, new DateInterval("P1D"), $iterEnd) as $date) {
            ...
            $calendarMap[$row][$dayIterator] = [
                'text' => $date->format('d'),
                'callback_data' => sprintf('calendar-day-%d-%d-%d', $date->format('d'), $month, $year)
            ];
    ...
    public function getMonthsList(int $year): array
    {
        $listMap = [
            [
        ...
        for ($month = 1; $month <= 12; $month++) {
            $listMap[$row][] = [
                'text' => date('F', strtotime(sprintf('%d-%d-01', $year, $month))),
                'callback_data' => sprintf('calendar-month-%d-%d', $month, $year)
            ];
    ...
    public function getYearsList(int $centerYear): array
    {
        $prevYear = $centerYear - 25;
        $nextYear = $centerYear + 25;
        $listMap = [
            [
                $prevYear <= 76 ? ['text' => ' ', 'callback_data' => 'null_callback'] : [
                    'text' => '<',
                    'callback_data' => 'calendar-years_list-' . $prevYear
                ],
                $nextYear >= 10024 ? ['text' => ' ', 'callback_data' => 'null_callback'] : [
                    'text' => '>',
                    'callback_data' => 'calendar-years_list-' . $nextYear
                ],
            ],
        ];
